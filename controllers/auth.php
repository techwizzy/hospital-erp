...
class Auth extends MY_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
             ...
             $this->data['InpatientchatData'] = $this->report_model->getInpatientChartData();
             $this->data['OutPatientchatData'] = $this->report_model->getOutpatientChartData();
             $this->data['ClinicPatientchatData'] = $this->report_model->getClinicChartData();
	...
	function activate($id, $code=false)
	{
		...
		if ($activation)
		{
			...
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			redirect("auth/getUsers", 'refresh');
		}
		else
		{
	...
	function create_user()
    {
        ...
        $this->form_validation->set_rules('nid', $this->lang->line('create_user_id'), 'min_length[7]|max_length[8]|numeric');


        if($this->input->post('is_allowed')=='yes')
        {
            $this->form_validation->set_rules('username', $this->lang->line('username'), 'required|is_unique[' . $tables['user'] . '.Username]');
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            $additional_data = array(
                ...
                'Allowed_login' => $this->input->post('is_allowed'),
                'active'        => $active_token,
                'Title'        => $this->input->post('group'),
                //payroll details
