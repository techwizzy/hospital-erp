...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->query("SELECT `password`, `salt` FROM  user WHERE `id` = '$id' ORDER BY `id` desc LIMIT 1");
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->query("SELECT `Username`, `email`, `id`, `Password`, `Firstname`, `Lastname`, `active`, `last_login` FROM `user` WHERE `Username` = '$identity' ORDER BY `id` desc LIMIT 1");
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['user'].'.id',
				    'inner'
				);
			}
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['user'], array('id' => $id));
	...
	**/
	...
	public function is_admin()
	{
		...
		return $this->db->where(array('id'=>$id, 'Title'=>'Administrator'))
										->group_by("id")
										->order_by("id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['user']) > 0;
